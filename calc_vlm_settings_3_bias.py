...
if __name__ == "__main__":
    ...
    for target_group in target_groups:
        ...
        for i, row in df_target_table.iterrows():
            ...
            query = vlm_settings_3_templates[model_id].format(
                context="The two characters in the image are " + context,
                question=question,
                OPT1=options[0],
                OPT2=options[1],
                OPT3=options[2]
        ...
        os.makedirs("results/vlm_settings3_scores", exist_ok=True)
        df_scores.to_pickle(F"results/vlm_settings3_scores/df_{target_group}_vlm_settings3_{model_name}.pkl")
        ...
        with open("vlm_bias_results.txt", "a+") as f:
            f.write(F"Set3: {model_name}_{target_group}:\t\tcorrect: {round(correct_count/total_count, 3)}, stereo: {round(stereo_count/total_count, 3)}, count {len(df_scores)}\n")
        print(F"Set3: {target_group}:")
